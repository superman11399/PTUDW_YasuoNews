  ...
  $(function () {
      ...
      $(this).validate({
        ...
        submitHandler: function (el) {
          ...
          $.getJSON(formURL, formData, function (res) {
            res.text =
              res.msg.charAt(0) === "0" ? res.msg.substring(4) : res.msg;
            res.fa =
              res.result === "error" && res.msg.charAt(0) === "0"
                ? "fa-close"
                : "fa-check";
    ...
    $headerSearchForm.on("click", ".btn", function (e) {
      ...
      setTimeout(function () {
        ...
        $document.on("click.headerSearchForm", function (e) {
          ...
          if (
            e.$target.not(".header--search-form").length === 0 ||
            e.$target.parents(".header--search-form").length === 0
          ) {
    ...
    if ($stickyContent.length) {
      $stickyContent.theiaStickySidebar({
        additionalMarginTop: $headerNavbar.length
          ? $headerNavbar.outerHeight()
          : 0,
      });
    }
    ...
    $.fn.initMap = function () {
      ...
      if (typeof google !== "undefined") {
        map = new google.maps.Map($el[0], {
          center: {
            lat: $el.data("map-latitude"),
            lng: $el.data("map-longitude"),
          },
    ...
    $postMap.on("click", ".btn-link", function () {
      ...
      setTimeout(function () {
        $document.on("click.postMap", function (e) {
          ...
          if (
            e.$target.not(".map--wrapper").length === 0 ||
            e.$target.parents(".map--wrapper").length === 0
          ) {
    ...
    if (isRTL && $footerSocialBg.length) {
      $footerSocialBg.css("right", function () {
        return (
          $footerCopyright.outerWidth() -
          ($footerSocial.position().left + $footerSocial.outerWidth())
        );
      });
  ...
  $wn.on("load", function () {
    ...
    $headerDropdownMenu.each(function () {
      var $t = $(this),
        $parent = $t.parent("li"),
        space =
          $parent.parents(".container").width() -
          ($parent.position().left - 15),
